...
public class VRTrackerTag : MonoBehaviour {
	...
	protected virtual void Start () {
        ...
        if(rotatingObject.Count > 0)
        {
            uniformRotation = false;
        }
	...
	protected virtual void FixedUpdate () {
		...
		if (orientationEnabled) {
            ...
            {
                //Apply rotation to the desired part of the gameObject
                for (int i = 0; i < rotatingObject.Count; i++)
                {
                    rotatingObject[i].gameObject.transform.rotation = tagRotation;
                }
            }
        }
